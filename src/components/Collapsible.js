...
import { ifIphoneX } from '../common/device';
...
export default class CollapsibleToolbar extends Component {
  ...
  constructor(props) {
    ...
    const ANDROID_STATUS_BAR_HEIGHT = props.translucentStatusBar
      ? StatusBar.currentHeight
      : 0;

    this.statusBarHeight =
      Platform.OS === 'ios' ? ifIphoneX(44, 20) : ANDROID_STATUS_BAR_HEIGHT;
    this.navBarHeight = APPBAR_HEIGHT + this.statusBarHeight;
    this.maxScrollableHeight = props.toolBarHeight - this.navBarHeight;

    const inputRange1 = [
      this.maxScrollableHeight / 2,
      this.maxScrollableHeight
    ];
    const inputRange2 = [
      this.maxScrollableHeight - 0.1,
      this.maxScrollableHeight
    ];
